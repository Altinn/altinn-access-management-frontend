...
namespace Altinn.AccessManagement.UI.Integration.Clients
{
    ...
    public class AccessManagementClientV0 : IAccessManagementClientV0
    {
        ...
        public async Task<List<AuthorizedParty>> GetReporteeListForUser()
        {
            try
            {
                string endpointUrl = $"authorizedparties?includeAltinn2=true&includeRoles=false&includeAccessPackages=false&includeResources=false&includeInstances=false&includePartiesViaKeyRoles=auto";
                string token = JwtTokenUtil.GetTokenFromContext(_httpContextAccessor.HttpContext, _platformSettings.JwtCookieName);
