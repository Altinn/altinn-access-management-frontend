...
| `<RestoreFocusFallback>` | wrap something durable (a list heading, a modal's content area) to catch focus. This happens when the requested id, and its `fallbackId` if any, are not present. Render exactly one per zone. |
