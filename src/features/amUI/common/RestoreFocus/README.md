...
| `useRestoreFocusOnDataChange(data)` | need a success callback to request focus on an id that will only exist after a query cache (`data`) refetches. For example, revoke moves an item from one query bucket to another. Call the returned `requestFocusOnDataChange(id, fallbackId?)` right after the action succeeds. It waits for `data` to become a new reference before resolving the focus request. If the calling component unmounts first (e.g. deleting the last item removed the whole list), the pending request is handed to the provider on the way out, so the zone's fallback chain still catches it. |
