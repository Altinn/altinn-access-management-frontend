...
LANGUAGES.forEach((language) => {
  test.describe(`Samtykke - fra person til org (${language})`, () => {
    ...
    test(`Standard samtykke`, async ({ login, consentPage, runAccessibilityTest }, testInfo) => {
    ...
    test(`Krav-template`, async ({ page, consentPage, login, runAccessibilityTest }, testInfo) => {
      const from = { pid: pickRandom(fromPersons) };
      const to = { orgNo: pickRandom(toOrgs) };
      const validTo = addTimeToNowUtc({ days: 2 });
      const api = new ConsentApiRequests(to.orgNo);
    ...
    test(`Fullmakt utføre tjeneste`, async ({
      ...
      await test.step('Approve consent', async () => {
        ...
        await runAccessibilityTest.scan(testInfo, 'fullmakt-før-godkjenning');
        await consentPage.approveFullmaktAndWaitLogout(APPROVED_REDIRECT_URL);
      });
    });

    test(`Lånesøknad`, async ({ consentPage, page, login, runAccessibilityTest }, testInfo) => {
      const from = { pid: pickRandom(fromPersons) };
      const to = { orgNo: pickRandom(toOrgs) };
      const validTo = addTimeToNowUtc({ days: 2 });
      const api = new ConsentApiRequests(to.orgNo);
    ...
    test(`Avvis samtykke`, async ({ consentPage, login, runAccessibilityTest }, testInfo) => {
      const from = { pid: pickRandom(fromPersons) };
      const to = { orgNo: pickRandom(toOrgs) };
      const validTo = addTimeToNowUtc({ days: 2 });
      const api = new ConsentApiRequests(to.orgNo);
      ...
      await test.step('Reject consent', async () => {
        ...
        await runAccessibilityTest.scan(testInfo, 'samtykke-før-avvisning');
